...
export default function DashboardReports() {
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      case 'inventory': return <Badge variant="outline">Inventory</Badge>;
      case 'forecast': return <Badge variant="outline">Forecast</Badge>;
      case 'purchase_order': return <Badge variant="outline">Purchase Order</Badge>;
      case 'analytics': return <Badge variant="outline">Analytics</Badge>;
  ...
  const getScheduleBadge = (schedule: string) => {
    switch (schedule) {
      ...
      case 'weekly': return <Badge variant="success">Weekly</Badge>;
      case 'monthly': return <Badge variant="success">Monthly</Badge>;
      default: return <Badge variant="outline">Unknown</Badge>;
    }
  };
  ...
  const totalSize = reports.reduce((sum, r) => sum + parseFloat(r.size.replace(' MB', '')), 0);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Ready Downloads</p>
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-xs">{formatDate(report.lastGenerated)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Created By</p>
                        <p className="font-medium text-xs">{report.createdBy}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Recipients</p>
                        <p className="font-medium text-xs">{report.recipients.length} recipient{report.recipients.length !== 1 ? 's' : ''}</p>
                      </div>
                    </div>
