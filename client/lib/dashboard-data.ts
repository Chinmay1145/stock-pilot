  ...
  skus.forEach((sku, skuIndex) => {
    for (let week = 0; week < 12; week++) {
      ...
      const predictedDemand = Math.max(
        0,
        Math.floor(baseDemand + seasonality + noise),
      );
      ...
      data.push({
        ...
        actualDemand:
          week < 4
            ? Math.floor(predictedDemand + (Math.random() - 0.5) * 10)
            : undefined,
...
export function generateMockPurchaseOrders(): PurchaseOrderData[] {
  ...
  const statuses: PurchaseOrderData["status"][] = [
    "pending",
    "approved",
    "ordered",
    "received",
  ...
  return products.map((product, index) => {
    ...
    const expectedDelivery = new Date(
      createdAt.getTime() + (7 + Math.random() * 14) * 86400000,
    );
    ...
    return {
      ...
      notes:
        Math.random() > 0.7 ? "Urgent order - low stock levels" : undefined,
    };
  });
}
